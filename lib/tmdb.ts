import { MediaType, Movie, TVShow, Credits } from '@/types';
...
export async function getAiringTodaySeriesFiltered(
  ...
  while (allResults.length < minResults && attemptsLeft > 0) {
    try {
      ...
      const filteredResults = data.results.filter((show: TVShow) => {
        ...
        if (show.genre_ids.some((id: number) => excludedGenreIds.includes(id))) {
          return false;
        }
  ...
  if (allResults.length < minResults) {
    try {
      ...
      const filteredPopular = popularData.results.filter((show: TVShow) => {
        if (!show.genre_ids || show.genre_ids.some((id: number) => excludedGenreIds.includes(id))) {
          return false;
        }
        return true;
      });
        // Ajouter uniquement les séries qui ne sont pas déjà dans allResults
      const existingIds = new Set(allResults.map((show: TVShow) => show.id));
      const additionalShows = filteredPopular.filter((show: TVShow) => !existingIds.has(show.id));
 ...
 * @returns TMDBResponse with trending movies
 */
export async function getTrendingMovies(page = 1) {
  const safePage = ensureSafePage(page);
  const data = await fetchFromTMDB<TMDBResponse<MediaItem>>(
    `/trending/movie/week?api_key=${TMDB_API_KEY}&page=${safePage}`
  );
  data.total_pages = Math.min(data.total_pages, TMDB_MAX_PAGE);
  return data;
}

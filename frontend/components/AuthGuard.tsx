...
interface AuthGuardProps {
  ...
  redirectTo?: string;
  showFallback?: boolean; // Option pour afficher le fallback au lieu de rediriger
}
 ...
 * Exemples d'usage :
 * 
 * 1. Redirection automatique (défaut) :
 * <AuthGuard>
 *   <ProtectedContent />
 * </AuthGuard>
 * 
 * 2. Avec fallback au lieu de redirection :
 * <AuthGuard showFallback>
 *   <ProtectedContent />
 * </AuthGuard>
 */
const AuthGuard: React.FC<AuthGuardProps> = ({ 
  ...
  // 🚀 REDIRECTION AUTOMATIQUE - useEffect pattern
  useEffect(() => {
    if (hasMounted && initialized && !loading && !isAuthenticated && !showFallback) {
      console.log('🚨 [AuthGuard] Redirecting to login - User not authenticated');
      router.push(redirectTo);
    }
  }, [hasMounted, initialized, loading, isAuthenticated, showFallback, redirectTo, router]);
  ...
  if (!isAuthenticated) {
    if (showFallback) {
      ...
      // Redirection en cours - afficher un loading
